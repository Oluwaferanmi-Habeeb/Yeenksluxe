    ...
    <header className="hero" aria-labelledby="hero-title">
      <div className="hero-stage">
        {slides.map((slide, index) => (
          <Image key={slide.src} src={slide.src} alt={index === activeSlide ? slide.label : ''} aria-hidden={index !== activeSlide} fill priority={index === 0} className={`hero-image ${index === activeSlide ? 'active' : ''}`} sizes="(max-width: 780px) 100vw, 100vw" />
        ))}
