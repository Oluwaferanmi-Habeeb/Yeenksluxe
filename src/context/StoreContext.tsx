...
function cleanOrderValue(value: string, maxLength: number) {
  return value
    .normalize('NFKC')
    .replace(/[\u0000-\u001F\u007F]/g, ' ')
    .replace(/[*_~`]/g, '')
...
export function StoreProvider({ children }: { children: ReactNode }) {
  const { user, profile, recordOrderReference } = useAccount();
  ...
  useEffect(() => {
    if (!user) return;
    const timer = window.setTimeout(() => setCheckoutForm(current => ({
      ...current,
  ...
  const getWhatsAppLink = (reference = orderReference ?? createOrderReference()) => {
    ...
    const customer = {
      name: cleanOrderValue(checkoutForm.name, 80),
      phone: cleanOrderValue(checkoutForm.phone, 32),
      email: cleanOrderValue(checkoutForm.email, 254),
      address: cleanOrderValue(checkoutForm.address, 160),
      city: cleanOrderValue(checkoutForm.city, 80),
      notes: cleanOrderValue(checkoutForm.notes, 220),
    };
  ...
  const handlePlaceOrder = (event: React.FormEvent) => {
    ...
    void recordOrderReference(reference);
